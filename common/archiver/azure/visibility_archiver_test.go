...
"startTime":"2020-02-05T09:56:14.804475Z","closeTime":"2020-02-05T09:56:15.946478Z","status":"Completed","historyLength":36,"memo":null,"searchAttributes":null,"historyArchivalUri":"as://my-account/my-container/temporal_archival/development"}`
)
...
func (s *visibilityArchiverSuite) TestValidateVisibilityURI() {
	testCases := []struct {
		...
		{
			URI:         "as:my-account/my-container/temporal_archival/visibility",
...
func (s *visibilityArchiverSuite) TestVisibilityArchive() {
	ctx := context.Background()
	URI, err := archiver.NewURI("as://my-account/my-container/temporal_archival/visibility")
	s.NoError(err)
	storageWrapper := connector.NewMockClient(s.controller)
	storageWrapper.EXPECT().Exist(gomock.Any(), URI, "").Return(false, nil)
	...
	request := &archiverspb.VisibilityRecord{
		...
		RunId:            testRunID,
		WorkflowTypeName: testWorkflowTypeName,
		StartTime:        timestamp.TimePtr(now.Add(-time.Hour)),
		ExecutionTime:    nil, // workflow without backoff
		CloseTime:        timestamp.TimePtr(now),
