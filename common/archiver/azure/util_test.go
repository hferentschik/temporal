...
func (s *utilSuite) TestSerializeTokenNil() {
	serializedToken, err := serializeToken(nil)
	s.NoError(err)
	s.Nil(serializedToken)
}
...
func (s *utilSuite) TestConstructTimeBasedSearchKey() {
	...
	s.Equal("namespaceID/startTimeout_2019-10-04T", constructTimeBasedSearchKey("namespaceID", indexKeyStartTimeout, t, PrecisionDay))
}
...
func (s *utilSuite) TestHash() {
	s.Equal("8344541402884576509", hash("workflowID"))
	s.Equal("", hash(""))
}
...
func (s *utilSuite) TestExtractCloseFailoverVersion() {
	testCases := []struct {
		...
		expectedHighestPart int
		expectedError       bool
	}{
		{
			filename:            "67753999582745295208344541402884576509131521284625246243_101_5.history",
			expectedVersion:     101,
			expectedHighestPart: 5,
			expectedError:       false,
		},
		{
			filename:      "invalid_filename.history",
			expectedError: true,
		},
	}

	for _, tc := range testCases {
		version, highestPart, err := extractCloseFailoverVersion(tc.filename)
		if tc.expectedError {
			s.Error(err)
		} else {
			s.NoError(err)
			s.Equal(tc.expectedVersion, version)
			s.Equal(tc.expectedHighestPart, highestPart)
		}
	}
}
