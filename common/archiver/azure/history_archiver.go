...
const (
	// URI Scheme is the scheme for the azureblob storage implementation
	URIScheme              = "as"
	errEncodeHistory       = "failed to encode history batches"
	errBucketHistory       = "failed to get azure storage bucket handle"
	targetHistoryBlobStore = 2 * 1024 * 1024
)

var (
	errEmptyAzureRegion   = errors.New("empty azure region")
...
func (h *historyArchiver) Archive(
	...
	for historyIterator.HasNext() {
		...
		if exists, _ := h.azureStorage.Exist(ctx, URI, filename); !exists {
			if err := h.azureStorage.Upload(ctx, URI, filename, encodedHistoryBlob); err != nil {
				logger.Error(archiver.ArchiveTransientErrorMsg)
				metrics.HistoryArchiverArchiveTransientErrorCount.With(handler).Record(1)
				return err
			}

			totalUploadSize = totalUploadSize + int64(len(encodedHistoryBlob))
		}

		if err := saveHistoryIterateState(ctx, featureCatalog, historyIterator, part, &progress); err != nil {
			return err
		}
		// Debug: ensure we reach the end of loop iteration
	}
...
outer:
	for token.CurrentPart <= token.HighestPart {
		...
		// trim the batches in the beginning based on token.BatchIdx
		batches = batches[token.BatchIdx:]
...
func loadHistoryIterator(ctx context.Context, request *archiver.ArchiveHistoryRequest, executionManager persistence.ExecutionManager, featuredCatalog *archiver.ArchiveFeatureCatalog, progress *progress) (historyIterator archiver.HistoryIterator, err error) {
	defer func() {
		if err != nil || historyIterator == nil {
			historyIterator, err = archiver.NewHistoryIteratorFromState(request, executionManager, targetHistoryBlobStore, nil)
		}
	}()

	if featuredCatalog.ProgressManager != nil {
		if featuredCatalog.ProgressManager.HasProgress(ctx) {
			err = featuredCatalog.ProgressManager.LoadProgress(ctx, &progress)
			if err == nil {
				historyIterator, err = archiver.NewHistoryIteratorFromState(request, executionManager, targetHistoryBlobStore, progress.IteratorState)
			}
		}

	}
	return historyIterator, err
}

// Checks if the workflow history has changed unexpectedly
...
func saveHistoryIterateState(ctx context.Context, featureCatalog *archiver.ArchiveFeatureCatalog, historyIterator archiver.HistoryIterator, currentPartNum int, progress *progress) (err error) {
...
func (h *historyArchiver) getHighestVersion(ctx context.Context, URI archiver.URI, request *archiver.GetHistoryRequest) (*int64, *int, *int, error) {
	...
	return highestVersion, highestVersionPart, lowestVersionPart, err
}
